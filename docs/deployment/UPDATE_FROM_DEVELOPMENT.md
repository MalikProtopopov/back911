...
## ⚠️ ВАЖНО: Эти команды выполняются ТОЛЬКО на сервере!

**Не выполняйте эти команды на локальной машине!**

- На локальной машине используется `.env` для разработки
- На сервере должен быть создан файл `.env.prod` (на основе `config/env.prod.template`)
- Файл `.env.prod` НЕ должен быть в git репозитории (он в `.gitignore`)
